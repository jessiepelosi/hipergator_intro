...
The amount of computing resources you have access to on both the regular and burst queues depends on your research group (specified by `account` and `qos`). These resources are shared by everyone in the research group, so if you are planning on running something that requires a lot of memory, it is probably a good idea to check the current resources available (see below under `squeue`) and also to check in with other members of your group so that you don't shut them out of running jobs for days.
